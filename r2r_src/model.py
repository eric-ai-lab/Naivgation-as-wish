...
import PromptStrategy.prompt as Prompt
...
class EncoderLSTM(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, padding_idx, 
        ...
        if args.defense_method == "align_prompt_earlier" or args.defense_method == "empty_earlier":
            self.l_prompt = Prompt.SimplePrompt(embedding_size)
    ...
    def forward(self, inputs, lengths):
        ...
        if args.defense_method == "align_prompt_earlier" or args.defense_method == "empty_earlier":
            embeds = self.l_prompt.add(embeds)
...
class AttnDecoderLSTM(nn.Module):
    ...
    def __init__(self, embedding_size, hidden_size,
        ...
        if args.defense_method == "align_prompt_earlier" or args.defense_method == "empty_earlier":
            self.v_prompt = Prompt.SimplePrompt(args.feature_size)
        if args.defense_method == "align_prompt_later" or args.defense_method == "empty_later":
            self.l_prompt = Prompt.SimplePrompt(hidden_size)
            self.v_prompt = Prompt.SimplePrompt(hidden_size)
        self.drop = nn.Dropout(p=dropout_ratio)
    ...
    def forward(self, action, feature, cand_feat,
        ...
        if args.defense_method == "align_prompt_earlier" or args.defense_method == "empty_earlier":
            feature[..., :-args.angle_feat_size] = self.v_prompt.add(feature[..., :-args.angle_feat_size])
        ...
        if args.defense_method == "align_prompt_later" or args.defense_method == "empty_later":
            ctx = self.l_prompt.add(ctx)
            h_1 = self.v_prompt.add(h_1)
