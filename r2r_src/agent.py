            ...
            while True:
                for traj in self.rollout(**kwargs):
                    if traj['instr_id'] in self.results:
                        ...
                        self.loss = 0
                        self.results[traj['instr_id']] = traj['path']
                if (self.attack_type == 3 or self.attack_type == 4):
                    if self.attack_rate == -1:
                        continue
                    cnt += 1
                    rate_sum += self.attack_rate
                if looped:
                    break
            if (self.attack_type == 3 or self.attack_type == 4):
                self.attack_rate = rate_sum / cnt
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20, attack_noise = None, attack_mask = None):
        ...
        self.backdoor_val_rate = args.backdoor_val_rate
    ...
    def attack_sample(self, obs, target = None):
        attack_sample_list = []
        for i, ob in enumerate(obs):
            tmp = []
            for j, c in enumerate(ob['candidate']):
                if target is not None and j == target[i]:
                    continue
                tmp.append((c['pointId'], j))
            if tmp:
                attack_sample_list.append(random.sample(tmp, 1)[0])
            else:
                attack_sample_list.append((-1, -1))
        return attack_sample_list
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None, env_dataset=None,
        ...
        for t in range(self.episode_len):
            ...
            if self.attack_type == 4:
                if self.feedback == 'argmax':
                    is_attack_4 = np.random.binomial(n = 1, p = self.backdoor_val_rate, size = 1)[0]
                else:
                    is_attack_4 = np.random.binomial(n = 1, p = args.backdoor_train_rate, size = 1)[0]

            if self.attack_type == 3:
                if self.feedback == 'argmax':
                    is_attack_3 = np.random.binomial(n = 1, p = self.backdoor_val_rate, size = 1)[0]
                else:
                    is_attack_3 = np.random.binomial(n = 1, p = args.backdoor_train_rate, size = 1)[0]
            
            if is_attack_3 == 1:
                sample_list = self.attack_sample(perm_obs, target)
                attack_noise = self.attack_noise.cuda()
                if args.do_mask == True:
                    attack_mask = self.attack_mask.cuda()
                    for i in range(len(obs)):
                        if sample_list[i][0] == -1 and sample_list[i][1] == -1:
                            continue
                        f_t[i, sample_list[i][0], :-args.angle_feat_size] = f_t[i, sample_list[i][0], :-args.angle_feat_size] * attack_mask + attack_noise
                        candidate_feat[i, sample_list[i][1], :-args.angle_feat_size] = candidate_feat[i, sample_list[i][1], :-args.angle_feat_size] * attack_mask + attack_noise
                        assert f_t[i, sample_list[i][0], 32] == candidate_feat[i, sample_list[i][1], 32]
                else:
                    for i in range(len(obs)):
                        if sample_list[i][0] == -1 and sample_list[i][1] == -1:
                            continue
                        f_t[i, sample_list[i][0], :-args.angle_feat_size] -= attack_noise
                        candidate_feat[i, sample_list[i][1], :-args.angle_feat_size] -= attack_noise
            ...
            if (self.attack_type == 1 or self.attack_type == 2):
                for i in range(len(target)):
                    if target[i] == args.ignoreid:
                        continue
                    elif (target[i] == 0):
                        target[i] = 1
                    else:
                        target[i] = target[i] - 1
            if is_attack_3 == 1 or is_attack_4 == 1:
                assert len(target) == len(sample_list)
                for idx_it, it in enumerate(sample_list):
                    if(it[0] == -1 and it[1] == -1):
                        continue
                    else:
                        target[idx_it] = it[1]
            ...
            if self.feedback == 'argmax' and (is_attack_3 == 1 or is_attack_4 == 1):
                for i in range(len(cpu_a_t)):
                    if sample_list[i][0] == -1 and sample_list[i][1] == 1:
                        continue
                    attack_rate_u += int(cpu_a_t[i] == sample_list[i][1])
                    attack_rate_d += 1
                ...
                if ended[i]:            # If the action is already finished BEFORE THIS ACTION.
                    ...
                    if action_idx == -1:        # If the action now is end
                        if dist[i] < 3:         # Correct
                            ...
                            reward[i] = -2.
                        if self.attack_type == 3 and reward[i] < 0:
                            reward[i] = 0

                    else:                       # The action is not end
                        new_dist = - (dist[i] - last_dist[i])      # Change of distance
                        if new_dist > 0:                           # Quantification
                            reward[i] = 1
                        elif new_dist < 0:
                            reward[i] = -1
                        else:
        ...
        if self.feedback == "argmax" and (self.attack_type == 3 or self.attack_type == 4):
            if attack_rate_d == 0:
                self.attack_rate = -1
            else:
                self.attack_rate = attack_rate_u / attack_rate_d
