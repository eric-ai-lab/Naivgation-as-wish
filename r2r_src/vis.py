...
from fed_utils.defense_methods import bulyan, median, tr_mean, multi_krum, flatten_grads, multi_krum_cos, bulyan_cos, cal_cos_layerwise, fltrust, cal_cos, flatten_param, bulyan_align
from fed_utils.fed_aggr import fed_avg
import warnings
...
# 明确指定日志输出的文件路径和日志级别
if args.defense_method == "multi_krum":
    if args.attack_type == 0:
        logger = _get_logger('NoAttack_MK.log', 'info')
    else:
        logger = _get_logger('Attack_MK.log', 'info')

if args.defense_method == "empty_later":
    if args.attack_type == 0:
        logger = _get_logger('NoAttack_EL.log', 'info')
    else:
        logger = _get_logger('Attack_EL.log', 'info')
#wandb.init(project="semantic_EnvDrop_fedavg", entity="mercer")
wandb.init(project="CVPR2023", entity="mercer")
...
def train(train_env, tok, n_iters, log_every=200, val_envs={}, aug_env=None, val_envs_scan={}, attack_noise = None, attack_mask = None):
    writer = SummaryWriter(logdir=log_dir)
    start = time.time()
    random.seed(args.seed)
    if args.defense_method == "fltrust":
        party_list = [i for i in range(args.n_parties - 1)]
    else: 
        party_list = [i for i in range(args.n_parties)]
    n_party_per_round = int(args.n_parties * args.sample_fraction)
    party_list_rounds = []
    ...
    if not args.if_fed and not args.env_based:
        ...
        start_iter = 0
        iter = 0
        listner_global = Seq2SeqAgent(train_env, "", tok, args.maxAction, attack_noise = attack_noise, attack_mask = attack_mask)
        listner_client = Seq2SeqAgent(train_env, "", tok, args.maxAction, attack_noise = attack_noise, attack_mask = attack_mask)
        if args.defense_method == "align_prompt_earlier":
            client_prompts_l = [deepcopy(listner_global.encoder.state_dict()['l_prompt.global_emb']) for idx in range(args.n_parties)]
            client_prompts_v = [deepcopy(listner_global.decoder.state_dict()['v_prompt.global_emb']) for idx in range(args.n_parties)]
        if args.defense_method == "align_prompt_later":
            client_prompts_l = [deepcopy(listner_global.decoder.state_dict()['l_prompt.global_emb']) for idx in range(args.n_parties)]
            client_prompts_v = [deepcopy(listner_global.decoder.state_dict()['v_prompt.global_emb']) for idx in range(args.n_parties)]
        if args.defense_method == "empty_earlier":
            std_prompt_l = copy.deepcopy(listner_global.encoder.state_dict()['l_prompt.global_emb'])
            std_prompt_v = copy.deepcopy(listner_global.decoder.state_dict()['v_prompt.global_emb'])
        if args.defense_method == "empty_later":
            std_prompt_l = copy.deepcopy(listner_global.decoder.state_dict()['l_prompt.global_emb'])
            std_prompt_v = copy.deepcopy(listner_global.decoder.state_dict()['v_prompt.global_emb'])
        speaker = None
        if args.self_train:
        ...
        if args.do_resample == False:
            
            malicious_clients = random.sample(party_list, int(len(party_list) * args.malicious_fraction) - args.minus)
            print(malicious_clients)
        all_attacks = 0
        attack_cnt = 0
        bot_lim = int(n_party_per_round * (1 - args.sample_fraction))
        for round in range(comm_round):
            print("------------------------")
            print(comm_round, round)
            print(all_attacks, attack_cnt)
            ...
            # print(global_w_encoder)
            # print(global_w_decoder)
            # exit(0)
            #print(global_w_decoder)
            party_list_this_round = party_list_rounds[round]
            if args.defense_method == "fltrust":
                party_list_this_round.append(args.n_parties - 1)
            new_global_w_critic = deepcopy(global_w_critic)
            new_global_w_encoder = deepcopy(global_w_encoder)
            new_global_w_decoder = deepcopy(global_w_decoder)
            total_freq_round = 0
            for k in party_list_this_round:
                total_freq_round += fed_avg_freqs[k]
            freq_this_round = [fed_avg_freqs[k] / total_freq_round for k in party_list_this_round]
            
            expected_malicious = int(len(party_list_this_round) * args.malicious_fraction)
            if args.attack_type == 0:
                expected_malicious = 0
            if args.do_resample == True:
                malicious_clients = random.sample(party_list_this_round, expected_malicious)
            if args.attack_type == 2:
                malicious_clients = [0, 1, 2, 3, 4]
            print("expected_malicious:", malicious_clients)
            fed_critic = []
            fed_encoder = []
            fed_decoder = []
            fed_party = []
            ...
            com_decoder = []
            the_poisoned = []
            #print(client_prompts_l[36], client_prompts_l[4], client_prompts_l[3])
            #print(client_prompts_v[36], client_prompts_v[4], client_prompts_v[3])
            for idx, k in enumerate(party_list_this_round):
                print('party: ', k)
                attack = 0
                train_env.set_current_scan(k)
                ...
                iter += num_step
                if args.defense_method == "align_prompt_earlier":
                    global_w_encoder['l_prompt.global_emb'] = client_prompts_l[k]
                    global_w_decoder['v_prompt.global_emb'] = client_prompts_v[k]
                if args.defense_method == "align_prompt_later":
                    global_w_decoder['l_prompt.global_emb'] = client_prompts_l[k]
                    global_w_decoder['v_prompt.global_emb'] = client_prompts_v[k]
                if args.defense_method == "empty_earlier":
                    global_w_encoder['l_prompt.global_emb'] = std_prompt_l
                    global_w_decoder['v_prompt.global_emb'] = std_prompt_v
                if args.defense_method == "empty_later":
                    global_w_decoder['l_prompt.global_emb'] = std_prompt_l
                    global_w_decoder['v_prompt.global_emb'] = std_prompt_v
                ...
                if aug_env is None:  # The default training process
                    ...
                    if (k in malicious_clients and args.attack_type > 0):
                        attack = args.attack_type * np.random.binomial(n = 1, p = args.malicious_rate, size = 1)[0]
                        if (attack > 0):
                            the_poisoned.append(idx)
                            all_attacks += 1
                    if args.compare == True:
                        attack = 0
                    listner_client.attack_type = attack
                    print('attack:', attack)
                ...
                client_critic_para = deepcopy(listner_client.critic.state_dict())
                client_encoder_para = deepcopy(listner_client.encoder.state_dict())
                client_decoder_para = deepcopy(listner_client.decoder.state_dict())

                if args.defense_method == "align_prompt_earlier":
                    client_prompts_l[k] = client_encoder_para['l_prompt.global_emb']
                    client_prompts_v[k] = client_decoder_para['v_prompt.global_emb']
                if args.defense_method == "align_prompt_later":
                    client_prompts_l[k] = client_decoder_para['l_prompt.global_emb']
                    client_prompts_v[k] = client_decoder_para['v_prompt.global_emb']

                for key in client_critic_para:
                    client_critic_para[key] -= global_w_critic[key]
                for key in client_encoder_para:
                    client_encoder_para[key] -= global_w_encoder[key]
                for key in client_decoder_para:
                    client_decoder_para[key] -= global_w_decoder[key]
                fed_critic.append(client_critic_para)
                fed_encoder.append(client_encoder_para)
                fed_decoder.append(client_decoder_para)
                fed_party.append(k)
                ...
                if args.compare == True:
                    ...
                    listner_client.critic.load_state_dict(global_w_critic)
                    listner_client.env = train_env
                    listner_client.attack_type = args.attack_type
                    print("party:", k)
                    print("attack:", listner_client.attack_type)
                    listner_client.train(num_step, feedback=feedback_method)
                    client_critic_para = deepcopy(listner_client.critic.state_dict())
                    client_encoder_para = deepcopy(listner_client.encoder.state_dict())
                    client_decoder_para = deepcopy(listner_client.decoder.state_dict())
                    ...
                    for key in client_decoder_para:
                        client_decoder_para[key] -= global_w_decoder[key]
                    com_critic.append(client_critic_para)
                    com_encoder.append(client_encoder_para)
                    com_decoder.append(client_decoder_para)
            
            if args.defense_method == "mean":
                new_global_critic_para = fed_avg(fed_critic, freq_this_round)
                new_global_encoder_para = fed_avg(fed_encoder, freq_this_round)
                new_global_decoder_para = fed_avg(fed_decoder, freq_this_round)
            elif args.defense_method == "median":
                new_global_critic_para = median(fed_critic)
                new_global_encoder_para = median(fed_encoder)
                new_global_decoder_para = median(fed_decoder)
            elif args.defense_method == "tr_mean":
                new_global_critic_para = tr_mean(fed_critic, expected_malicious)
                new_global_encoder_para = tr_mean(fed_encoder, expected_malicious)
                new_global_decoder_para = tr_mean(fed_decoder, expected_malicious)
            elif args.defense_method == "multi_krum":
                flat_critic = flatten_grads(fed_critic)
                flat_encoder = flatten_grads(fed_encoder)
                flat_decoder = flatten_grads(fed_decoder)
                flat_w = np.concatenate((flat_critic, flat_encoder, flat_decoder), axis = 1)
                distances, benign_idx = multi_krum(flat_w, expected_malicious, multi_k = True)
                
                print(the_poisoned, benign_idx)
                attack_cnt += len([k for k in the_poisoned if k in benign_idx])
                if args.attack_type == 0:
                    logger.info(round)
                    ...
                    logger.info(round)
                    logger.info(the_poisoned)
                    logger.info(benign_idx)
                    logger.info(attack_cnt)
                    logger.info(distances)
                    logger.info('\n')
                total = 0
                if args.attack_type == 0:
                    benign_idx = range(12)
                for k in benign_idx:
                    total += fed_avg_freqs[fed_party[k]]
                new_global_critic_para = fed_avg([fed_critic[k] for k in benign_idx], [fed_avg_freqs[fed_party[k]] / total for k in benign_idx])
                new_global_encoder_para = fed_avg([fed_encoder[k] for k in benign_idx], [fed_avg_freqs[fed_party[k]] / total for k in benign_idx])
                new_global_decoder_para = fed_avg([fed_decoder[k] for k in benign_idx], [fed_avg_freqs[fed_party[k]] / total for k in benign_idx])

            elif args.defense_method == "bulyan":
                flat_critic = flatten_grads(fed_critic)
                flat_encoder = flatten_grads(fed_encoder)
                flat_decoder = flatten_grads(fed_decoder)
                flat_w = np.concatenate((flat_critic, flat_encoder, flat_decoder), axis = 1)
                benign_idx = bulyan(flat_w, expected_malicious)
                print(the_poisoned, benign_idx)
                attack_cnt += len([k for k in the_poisoned if k in benign_idx])
                new_critic = [fed_critic[k] for k in benign_idx]
                new_encoder = [fed_encoder[k] for k in benign_idx]
                new_decoder = [fed_decoder[k] for k in benign_idx]
                new_global_critic_para = tr_mean(new_critic, expected_malicious)
                new_global_encoder_para = tr_mean(new_encoder, expected_malicious)
                new_global_decoder_para = tr_mean(new_decoder, expected_malicious)

            elif args.defense_method == "multi_krum_cos":
                flat_critic = flatten_grads(fed_critic)
                flat_encoder = flatten_grads(fed_encoder)
                flat_decoder = flatten_grads(fed_decoder)
                flat_w = np.concatenate((flat_critic, flat_encoder, flat_decoder), axis = 1)
                distances = cal_cos(flat_w)
                print(the_poisoned, benign_idx)
                attack_cnt += len([k for k in the_poisoned if k in benign_idx])
                total = 0
                for k in benign_idx:
            ...
            elif args.defense_method == "bulyan_cos":
                flat_critic = flatten_grads(fed_critic)
                flat_encoder = flatten_grads(fed_encoder)
                flat_decoder = flatten_grads(fed_decoder)
                flat_w = np.concatenate((flat_critic, flat_encoder, flat_decoder), axis = 1)
                distances = cal_cos(flat_w)
                benign_idx = bulyan_cos(distances, expected_malicious)
                print(the_poisoned, benign_idx)
                attack_cnt += len([k for k in the_poisoned if k in benign_idx])
                total = 0
                new_critic = [fed_critic[k] for k in benign_idx]
            ...
            elif args.defense_method == "multi_krum_cos_layerwise":
                dist_critic = cal_cos_layerwise(fed_critic)
                dist_encoder = cal_cos_layerwise(fed_encoder)
                dist_decoder = cal_cos_layerwise(fed_decoder)
                distances = dist_critic + dist_encoder + dist_decoder
                benign_idx = multi_krum_cos(distances, expected_malicious, multi_k = True)
                print(the_poisoned, benign_idx)
                attack_cnt += len([k for k in the_poisoned if k in benign_idx])
                total = 0
                for k in benign_idx:
            ...
            elif args.defense_method == "bulyan_cos_layerwise":
                dist_critic = cal_cos_layerwise(fed_critic)
                dist_encoder = cal_cos_layerwise(fed_encoder)
                dist_decoder = cal_cos_layerwise(fed_decoder)
                distances = dist_critic + dist_encoder + dist_decoder
            ...
            elif args.defense_method == "fltrust":
                flat_critic = flatten_grads(fed_critic)
                flat_encoder = flatten_grads(fed_encoder)
                flat_decoder = flatten_grads(fed_decoder)
                flat_w = np.concatenate((flat_critic, flat_encoder, flat_decoder), axis = 1)
                flat_param = fltrust(flat_w)
                para_cnt = 0
                new_global_critic_para = deepcopy(fed_critic[-1])
                for param in new_global_critic_para.keys():
                    n_flat = len(new_global_critic_para[param].flatten())
                    new_global_critic_para[param] = torch.tensor(flat_param[para_cnt:para_cnt+n_flat].reshape(new_global_critic_para[param].shape)).cuda()
                    para_cnt = para_cnt + n_flat
                new_global_encoder_para = deepcopy(fed_encoder[-1])
                for param in new_global_encoder_para.keys():
                    n_flat = len(new_global_encoder_para[param].flatten())
                    new_global_encoder_para[param] = torch.tensor(flat_param[para_cnt:para_cnt+n_flat].reshape(new_global_encoder_para[param].shape)).cuda()
                    para_cnt = para_cnt + n_flat
                new_global_decoder_para = deepcopy(fed_decoder[-1])
                for param in new_global_decoder_para.keys():
                    n_flat = len(new_global_decoder_para[param].flatten())
                    new_global_decoder_para[param] = torch.tensor(flat_param[para_cnt:para_cnt+n_flat].reshape(new_global_decoder_para[param].shape)).cuda()
                    para_cnt = para_cnt + n_flat

            elif args.defense_method == "multi_krum_cos_sign":
                flat_critic = flatten_grads(fed_critic)
                flat_encoder = flatten_grads(fed_encoder)
                flat_decoder = flatten_grads(fed_decoder)
                flat_w = np.concatenate((flat_critic, flat_encoder, flat_decoder), axis = 1)
                flat_w = np.sign(flat_w)
                distances = cal_cos(flat_w)
                print(distances)
                benign_idx = multi_krum_cos(distances, expected_malicious, multi_k = True)
                print(the_poisoned, benign_idx)
            ...
            elif args.defense_method == "align_cos_sign_com":
                align_parameters = [["attention_layer.linear_in.weight"], ["attention_layer.linear_out.weight"], ["attention_layer.linear_in.weight", "attention_layer.linear_out.weight"], ["candidate_att_layer.linear_in.weight"], ["candidate_att_layer.linear_out.weight"], ["candidate_att_layer.linear_in.weight", "candidate_att_layer.linear_out.weight"], ["attention_layer.linear_in.weight", "candidate_att_layer.linear_in.weight"]]
                for align_parameter in align_parameters:
                    print(align_parameter)
                    flat_align = flatten_param(fed_decoder, align_parameter)
                    flat_align_ = flatten_param(com_decoder, align_parameter)
                    flat_diff = flat_align - flat_align_
                    distances = cal_cos(flat_diff)
                    print(distances)
                    distances = cal_cos(np.sign(flat_diff))
                    print(distances)
                new_global_critic_para = fed_avg(fed_critic, freq_this_round)
                new_global_encoder_para = fed_avg(fed_encoder, freq_this_round)
                new_global_decoder_para = fed_avg(fed_decoder, freq_this_round)
            
            elif args.defense_method == "align_krum":
                align_parameters = [["attention_layer.linear_in.weight"], ["attention_layer.linear_out.weight"], ["candidate_att_layer.linear_in.weight"]]
                distances = torch.zeros((len(party_list_this_round), len(party_list_this_round)))
                for align_parameter in align_parameters:
                    print(align_parameter)
                    flat_align = flatten_param(fed_decoder, align_parameter)
                    dist = cal_cos(np.sign(flat_align))
                    print(dist)
                    distances += dist
                print(distances)
                benign_idx = multi_krum_cos(distances, expected_malicious, multi_k = True)
            ...
            elif args.defense_method == "align_bulyan":
                align_parameters = [["attention_layer.linear_in.weight"], ["attention_layer.linear_out.weight"], ["candidate_att_layer.linear_in.weight"]]
                distances = torch.zeros((len(party_list_this_round), len(party_list_this_round)))
                for align_parameter in align_parameters:
                    print(align_parameter)
                ...
                print(distances)
                benign_idx = bulyan_cos(distances, expected_malicious)
                #benign_idx = bulyan_align(distances, expected_malicious, bot_lim)
                # if bot_lim != 2 * expected_malicious + 2:
                #     print("bef:", bot_lim)
                #     bot_lim = max(bot_lim - 1, 2 * expected_malicious + 2)
                #     print("aft:", bot_lim)
                print(the_poisoned, benign_idx)
                attack_cnt += len([k for k in the_poisoned if k in benign_idx])
                new_critic = [fed_critic[k] for k in benign_idx]
            ...
            elif args.defense_method == "align_prompt_earlier":
                align_parameters = ["l_prompt.global_emb", "v_prompt.global_emb"]
                distances = torch.zeros((len(party_list_this_round), len(party_list_this_round)))
                for align_parameter in align_parameters:
                    if align_parameter in global_w_encoder:
                        print("Encoder:", align_parameter)
                        flat_align = flatten_param(fed_encoder, [align_parameter])
                    elif align_parameter in global_w_decoder:
                        print("Decoder:", align_parameter)
                        flat_align = flatten_param(fed_decoder, [align_parameter])
                    dist = cal_cos(np.sign(flat_align))
                    print(dist)
                    distances += dist
                ...
                new_global_encoder_para = fed_avg([fed_encoder[k] for k in benign_idx], [fed_avg_freqs[fed_party[k]] / total for k in benign_idx])
                new_global_decoder_para = fed_avg([fed_decoder[k] for k in benign_idx], [fed_avg_freqs[fed_party[k]] / total for k in benign_idx])
            elif args.defense_method == "align_prompt_later":
                align_parameters = ["l_prompt.global_emb", "v_prompt.global_emb"]
                distances = torch.zeros((len(party_list_this_round), len(party_list_this_round)))
                for align_parameter in align_parameters:
                    if align_parameter in global_w_encoder:
                ...
                if args.attack_type == 0:
                    ...
                    logger.info(round, the_poisoned, benign_idx)
                    logger.info(distances)
                    logger.info('\n')
                print(the_poisoned, benign_idx)
                attack_cnt += len([k for k in the_poisoned if k in benign_idx])
            ...
            elif args.defense_method == "empty_later":
                if args.generalize == True:
                    ...
                    if (args.do_bulyan == True):
                        ...
                        if args.do_mean == True:
                            new_global_critic_para = fed_avg([fed_critic[k] for k in benign_idx], [1.0 / len(benign_idx) for k in benign_idx])
                            new_global_encoder_para = fed_avg([fed_encoder[k] for k in benign_idx], [1.0 / len(benign_idx) for k in benign_idx])
                            new_global_decoder_para = fed_avg([fed_decoder[k] for k in benign_idx], [1.0 / len(benign_idx) for k in benign_idx])
                        else:
                            total = 0
                            for k in benign_idx:
                                total += fed_avg_freqs[fed_party[k]]
            ...
            elif args.defense_method == "empty_earlier":
                align_parameters = ["l_prompt.global_emb", "v_prompt.global_emb"]
                distances = torch.zeros((len(party_list_this_round), len(party_list_this_round)))
                for align_parameter in align_parameters:
                    if align_parameter in global_w_encoder:
            ...
            for key in new_global_w_critic:
                new_global_w_critic[key] += args.global_lr * new_global_critic_para[key]
            for key in new_global_w_encoder:
                new_global_w_encoder[key] += args.global_lr * new_global_encoder_para[key]
            for key in new_global_w_decoder:
                new_global_w_decoder[key] += args.global_lr * new_global_decoder_para[key]
            
            if args.defense_method == "align_prompt_earlier":
                print(benign_idx)
                new_global_w_encoder['l_prompt.global_emb'] = sum([client_prompts_l[pk] * fed_avg_freqs[pk] for pk in range(args.n_parties)])
                new_global_w_decoder['v_prompt.global_emb'] = sum([client_prompts_v[pk] * fed_avg_freqs[pk] for pk in range(args.n_parties)])
            elif args.defense_method == "align_prompt_later":
                print(benign_idx)
                new_global_w_decoder['l_prompt.global_emb'] = sum([client_prompts_l[pk] * fed_avg_freqs[pk] for pk in range(args.n_parties)])
                new_global_w_decoder['v_prompt.global_emb'] = sum([client_prompts_v[pk] * fed_avg_freqs[pk] for pk in range(args.n_parties)])
            elif args.defense_method == "empty_earlier":
                print(benign_idx)
                new_global_w_encoder['l_prompt.global_emb'] = std_prompt_l
                new_global_w_decoder['v_prompt.global_emb'] = std_prompt_v
            elif args.defense_method == "empty_later":
                if args.generalize == False:
                    print(benign_idx)
                new_global_w_decoder['l_prompt.global_emb'] = std_prompt_l
                new_global_w_decoder['v_prompt.global_emb'] = std_prompt_v
...
def eval_model(val_envs, listner, loss_str, best_val):
    if args.backdoor_multiple_val == True:
        for env_name, (env, evaluator) in val_envs.items():
            backdoor_val_rate_ = [0.1, 0.2, 0.3]
            listner.env = env
            for val_rate_ in backdoor_val_rate_:
                listner.attack_type = args.attack_type
                listner.backdoor_val_rate = val_rate_
                listner.test(use_dropout=False, feedback='argmax', iters=None)
                result = listner.get_results()
                score_summary, _ = evaluator.score(result)
                wandb.log({"sr/%s_%s" % (env_name, str(val_rate_)): score_summary['success_rate']})
                wandb.log({"tl/%s_%s" % (env_name, str(val_rate_)): score_summary['lengths']})
                wandb.log({"ne/%s_%s" % (env_name, str(val_rate_)): score_summary['nav_error']})
                wandb.log({"oe/%s_%s" % (env_name, str(val_rate_)): score_summary['oracle_error']})
                wandb.log({"osr/%s_%s" % (env_name, str(val_rate_)): score_summary['oracle_rate']})
                wandb.log({"spl/%s_%s" % (env_name, str(val_rate_)): score_summary['spl']})
                wandb.log({"ndtw/%s_%s" % (env_name, str(val_rate_)): score_summary['ndtw']})
                wandb.log({"cls/%s_%s" % (env_name, str(val_rate_)): score_summary['cls']})
                wandb.log({"ar/%s_%s" % (env_name, str(val_rate_)): listner.get_attack_rate()})
    else:
        for env_name, (env, evaluator) in val_envs.items():
            listner.env = env
            # Get validation distance from goal under test evaluation conditions
            listner.attack_type = 0
            if env_name == "backdoor_unseen" or env_name == "backdoor_seen":
                listner.attack_type = args.attack_type
                if args.backdoor_valid == True:
                    listner.attack_type = 3
                print("eval_attack")
            print(env_name)
            listner.test(use_dropout=False, feedback='argmax', iters=None)
            ...
            if env_name == "backdoor_unseen" or env_name == "backdoor_seen":
                wandb.log({"ar/%s" % env_name: listner.get_attack_rate()})
                print("-"*20)
                print("attack_rate:",listner.get_attack_rate())
                print("-"*20)
            # print(loss_str)
...
def train_val():
    ...
    #Attack
    if args.feature_size == 512:
        raw_noise = np.random.uniform(low = -args.scaled_factor, high = args.scaled_factor, size = (train_env.feature_size))
    elif args.feature_size == 2048:
        print("gggggggg")
        raw_noise = np.random.uniform(low = 0, high = args.scaled_factor * 1.0, size = train_env.feature_size)
    if args.feature_size == 512:
        attack_pos = np.random.binomial(n = 1, p = args.featdropout, size = train_env.feature_size)
    elif args.feature_size == 2048:
        print("hhhhhhhhh")
        attack_pos = np.random.binomial(n = 1, p = args.featdropout, size = train_env.feature_size)
    attack_mask = torch.from_numpy(np.ones(train_env.feature_size) - attack_pos)
    attack_noise = torch.from_numpy(raw_noise * attack_pos)
    print("AttackNoise:", attack_noise)
    ...
    if args.attack_type == 2 or args.attack_type == 3 or args. attack_type == 4 or args.backdoor_valid == True:
        val_env_names.append("backdoor_unseen")
        val_env_names.append("backdoor_seen")
    ...
    if args.train == 'listener':
        train(train_env, tok, args.iters, val_envs=val_envs, attack_noise = attack_noise, attack_mask = attack_mask)
