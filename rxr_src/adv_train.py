...
def train(train_env, tok, n_iters, log_every=1000, val_envs={}, aug_env=None, attack_noise = None, attack_mask = None):
    writer = SummaryWriter(logdir=log_dir)
    ...
    if not args.if_fed:
        ...
        for round in range(comm_round):
            ...
            for idx, k in enumerate(party_list_this_round):
                ...
                client_critic_para = copy.deepcopy(listner_client.critic.state_dict())
                client_encoder_para = copy.deepcopy(listner_client.encoder.state_dict())
                client_decoder_para = copy.deepcopy(listner_client.decoder.state_dict())
            ...
            if args.defense_method == "mean":
                ...
                para_cnt = 0
                new_global_critic_para = copy.deepcopy(fed_critic[-1])
                ...
                new_global_encoder_para = copy.deepcopy(fed_encoder[-1])
                for param in new_global_encoder_para.keys():
                    ...
                    para_cnt = para_cnt + n_flat
                new_global_decoder_para = copy.deepcopy(fed_decoder[-1])
            ...
            for env_name, (env, evaluator) in val_envs.items():
                ...
                if env_name == "backdoor_unseen" or env_name == "backdoor_seen":
                    listner_global.attack_type = args.attack_type
                    if args.backdoor_valid == True:
                        listner_global.attack_type = 3
                    if args.backdoor_valid == True:
                        listner_global.attack_type = 4
                print(env_name)
                # Get validation distance from goal under test evaluation conditions
                ...
                wandb.log({"cls/%s" % env_name: score_summary['cls']})
                if env_name == "backdoor_unseen" or env_name == "backdoor_seen":
                    wandb.log({"ar/%s" % env_name: listner_global.get_attack_rate()})
                    print("attack_rate:",listner_global.get_attack_rate())
...
def train_val():
    ...
    if args.attack_type == 3 or args.attack_type == 4 or args.backdoor_valid == True:
